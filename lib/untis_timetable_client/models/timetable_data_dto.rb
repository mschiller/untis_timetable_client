...
module UntisTimetableClient
  # The timegrids and periods of the requested timetable.
  class TimetableDataDto
    # Time grids referenced in timetable periods.
    attr_accessor :time_grids
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UntisTimetableClient::TimetableDataDto` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UntisTimetableClient::TimetableDataDto`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'time_grids')
        if (value = attributes[:'time_grids']).is_a?(Array)
          self.time_grids = value
        end
      end
    ...
    # @param [Object] time_grids Value to be assigned
    def time_grids=(time_grids)
      if time_grids.nil?
        fail ArgumentError, 'time_grids cannot be nil'
      end
