...
module UntisTimetableClient
  # Header data. Information about the content like school, filters, creation time etc.
  class HeaderDataDto
    # Timestamp in ISO 8601 format when this timetable data response was created.
    attr_accessor :created

    # Id of the WebUntis user who requested the timetable data.
    attr_accessor :user

    attr_accessor :school

    # Timestamp in ISO 8601 format of beginning of school year.
    attr_accessor :school_year_start

    # Timestamp in ISO 8601 format of end of school year.
    attr_accessor :school_year_end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UntisTimetableClient::HeaderDataDto` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UntisTimetableClient::HeaderDataDto`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'school_year_start')
        self.school_year_start = attributes[:'school_year_start']
      end

      if attributes.key?(:'school_year_end')
        self.school_year_end = attributes[:'school_year_end']
      end
    ...
    def list_invalid_properties
      ...
      if @user.to_s.length > 128
        invalid_properties.push('invalid value for "user", the character length must be smaller than or equal to 128.')
      end
    ...
    def valid?
      ...
      return false if @user.to_s.length > 128
    ...
    def user=(user)
      ...
      if user.to_s.length > 128
        fail ArgumentError, 'invalid value for "user", the character length must be smaller than or equal to 128.'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          user == o.user &&
          school == o.school &&
          school_year_start == o.school_year_start &&
          school_year_end == o.school_year_end &&
          filter == o.filter
    end
    ...
    def hash
      [created, user, school, school_year_start, school_year_end, filter].hash
    end
